	...
	var LockScreen = {
		...
		unlock: function( $lock ){
			...
			$lock.on("click",function(e){
				if($("#pwd").val() == window.sessionStorage.getItem("password")) _self.hide();
			...
			$.magnificPopup.open({
				...
				callbacks: {
					change: function() {
						...
						_self.unlock( this.content.find( '#testbtn' ) );
					}
				}
			});
		},
							...
							'<div class="panel panel-sign">',
								'<div class="panel-body">',
										...
										'<div class="current-user text-center">',
											...
											'<h2 id="LockUserName" class="user-name text-dark m-none">'+window.sessionStorage.getItem('stuname')+'</h2>',
											'<p  id="LockUserEmail" class="user-email m-none">系统管理员</p>',
										'</div>',
										'<div class="form-group mb-lg">',
											'<div class="input-group input-group-icon">',
												'<input id="pwd" name="pwd" type="password" class="form-control input-lg" placeholder="密码" />',
										...
										'<div class="row">',
											...
											'<div class="col-xs-6 text-right">',
												'<button id="testbtn" class="btn btn-primary">解锁</button>',
											'</div>',
										'</div>',
